import { ChainId, Currency, currencyEquals, JSBI, Price, WMATIC } from '../sdk'
...
export default function useUSDCPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    () => [
      ...
      [chainId ? WMATIC[chainId] : undefined, chainId === ChainId.MATIC ? USDC : undefined]
    ],
  ...
  return useMemo(() => {
    ...
    const ethPairETHAmount = ethPair?.reserveOf(WMATIC[chainId])
    const ethPairETHUSDCValue: JSBI =
      ethPairETHAmount && usdcEthPair ? usdcEthPair.priceOf(WMATIC[chainId]).quote(ethPairETHAmount).raw : JSBI.BigInt(0)
    ...
    if (ethPairState === PairState.EXISTS && ethPair && usdcEthPairState === PairState.EXISTS && usdcEthPair) {
      if (usdcEthPair.reserveOf(USDC).greaterThan('0') && ethPair.reserveOf(WMATIC[chainId]).greaterThan('0')) {
        const ethUsdcPrice = usdcEthPair.priceOf(USDC)
        const currencyEthPrice = ethPair.priceOf(WMATIC[chainId])
